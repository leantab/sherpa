...
class Core 
{
  ...
  {
    $this->match = $match;
    $this->stage = (int)($this->match->current_stage);
    $this->global = [];
    $this->company = [];
  ...
  public function process()
  {
    ...
    try{
      ...
      $this->global['annual_rounds'] = 12 / $this->match->match_parameters['accounting_period'];
      ...
      if($this->stage == 0){
        $this->global['interest_rate'] = $this->match->match_parameters['interest_rate'];
      }else{
        $this->global['interest_rate'] = $this->match->goverment_parameters['stage_' . $this->stage]['interest_rate'];
      }

      $arr_price = [];
      // Loop 1
      foreach ($this->match->ceos as $ceo) {
        if ($this->stage == 0) {
        ...
        $this->company[$ceo->id]['risk_level'] = round(min($this->match->match_parameters['risk_limit_max'] / 100, max(1 - $this->company[$ceo->id]['risk_score'] / 3.58, $this->match->match_parameters['risk_limit_min'] / 100)), 2);
        ...
        if($this->stage == 0){
          $this->ceo[$ceo->id]['price'] = round($this->match->match_parameters['price_t0_leverage'] * $this->industry['reference_cost']);
        ...
        $this->company[$ceo->id]['employees'] = max(round($this->company[$ceo->id]['ppe'] / $this->company[$ceo->id]['vpe'] * log($this->company[$ceo->id]['production'] / 10)), 0);
        ...
        $this->company[$ceo->id]['salaries_expense'] = $this->match->match_parameters['salary'] * $this->company[$ceo->id]['employees'] * $this->match->match_parameters['accounting_period'];
      ...
      $this->global['average_production'] = $this->global['production_sum'] / $this->match->ceos->count();
      $this->global['average_cbu'] = $this->global['cbu_sum'] / $this->match->ceos->count();

      $this->global['average_price'] = $this->global['price_sum'] / $this->match->ceos->count();
      ...
      foreach ($this->match->ceos as $ceo) {
        ...
        if ($this->stage == 0) {
          ...
          $this->company[$ceo->id]['active_mkt'] = $this->company[$ceo->id]['mkt'] + $ceo->pivot->results['stage_' . ($this->stage - 1)]['mkt'] * 0.25 + $this->match->results['stage_' . ($this->stage - 1)]['active_mkt_industry'] * 0.1;
        }
      ...
      foreach ($this->match->ceos as $ceo) {
        ...
        $this->company[$ceo->id]['mkt_index'] = max(log(($this->industry['p_mkt'] / $this->match->match_parameters['players']) + ($this->industry['p_mkt'] * $this->company[$ceo->id]['delta_mkt_share'])), 0);
      ...
      $this->global['pms'] = $this->match->match_parameters['pms'];

      if ($this->stage == 0) {
        $this->global['delta_interest_rate'] = $this->global['interest_rate'];
        $this->global['added_demand'] = $this->global['output_industry'] * (1 + ($this->match->match_parameters['initial_eq'] / 100));
        $this->global['real_added_demand'] = $this->match->match_parameters['initial_eq'];
      } else {
        $this->global['delta_interest_rate'] = $this->global['interest_rate'] - $this->match->results['stage_'. ($this->stage -1)]['interest_rate'];

        $this->global['added_demand'] = $this->match->results['stage_' . ($this->stage - 1)]['added_demand'] * (1 + $this->match->goverment_parameters['stage_'.$this->stage]['added_demand_variation'] / 100) * (1 - ($this->global['delta_interest_rate'] / 100 * $this->global['pms']));

        $this->global['real_added_demand'] = (($this->global['added_demand'] / $this->match->results['stage_' . ($this->stage - 1)]['added_demand']) - 1) * 100;
      ...
      foreach ($this->match->ceos as $ceo) {
        ...
        $this->company[$ceo->id]['loan_ratio'] = $this->match->match_parameters['loan_ratio'];
        if($this->stage == 0){
          $this->company[$ceo->id]['top_loan'] = $this->company[$ceo->id]['loan_ratio'] * ($this->global['total_revenue_industry'] / $this->match->match_parameters['players']);
        }else{
          $this->company[$ceo->id]['top_loan'] = $this->company[$ceo->id]['loan_ratio'] * ($this->match->results['stage_0']['total_revenue_industry'] / $this->match->match_parameters['players']);
        }
      ...
      foreach ($this->match->ceos as $ceo) {
        ...
        if ($this->stage == 0) {
          $this->company[$ceo->id]['opening_cash'] = $this->match->match_parameters['opening_cash_ratio'] * $this->company[$ceo->id]['ppe'];
          ...
          if($this->company[$ceo->id]['event_prob'] > 0){
            // EVENTO RANDOM
            $this->company[$ceo->id]['top_limit'] = round($this->match->match_parameters['players'] / ($this->company[$ceo->id]['event_prob'] / 100));
            $this->company[$ceo->id]['trigger'] = rand(1, $this->company[$ceo->id]['top_limit']);
          }else{
            $this->company[$ceo->id]['trigger'] = false;
          }
          
          if ($this->company[$ceo->id]['trigger'] > $this->match->match_parameters['players']) {
        ...
        if($this->stage == 0){
          $this->company[$ceo->id]['subsidy'] = ($this->company[$ceo->id]['delta_employees'] < 0) ? $this->company[$ceo->id]['delta_employees'] * $this->match->match_parameters['compensation_cost'] * $this->match->match_parameters['salary'] * $this->stage : $this->company[$ceo->id]['delta_employees'] * $this->match->match_parameters['compensation_cost'] * $this->match->match_parameters['salary'];
          $this->company[$ceo->id]['financial_result_multiplier'] = ($this->company[$ceo->id]['financial_debt'] > 0) ? ($this->global['interest_rate'] / 100) : ($this->global['interest_rate'] - $this->match->match_parameters['financial_cost']) / 100;
        }else{
          $this->company[$ceo->id]['subsidy'] = ($this->company[$ceo->id]['delta_employees'] < 0) ? $this->company[$ceo->id]['delta_employees'] * $this->match->goverment_parameters['stage_' . $this->stage]['compensation_cost'] * $this->match->match_parameters['salary'] * $this->stage : $this->company[$ceo->id]['delta_employees'] * $this->match->goverment_parameters['stage_' . $this->stage]['compensation_cost'] * $this->match->match_parameters['salary'];
          $this->company[$ceo->id]['financial_result_multiplier'] = ($this->company[$ceo->id]['financial_debt'] > 0) ? ($this->global['interest_rate'] / 100) : ($this->global['interest_rate'] - $this->match->goverment_parameters['stage_' . $this->stage]['financial_cost']) / 100;
        }
        ...
        if($this->stage == 0){
          $this->company[$ceo->id]['profit_tax_amount'] = ($this->company[$ceo->id]['uai'] <= 0) ? 0 : ($this->company[$ceo->id]['uai'] * -1) * ($this->match->match_parameters['profit_tax'] / 100);
          $this->company[$ceo->id]['vat_tax_amount'] = ($this->company[$ceo->id]['uai'] <= 0) ? 0 : - ($this->company[$ceo->id]['total_revenue'] - $this->company[$ceo->id]['total_cost']) * ($this->match->match_parameters['vat_tax'] / 100);
        }else{
          $this->company[$ceo->id]['profit_tax_amount'] = ($this->company[$ceo->id]['uai'] <= 0) ? 0 : ($this->company[$ceo->id]['uai'] * -1) * ($this->match->goverment_parameters['stage_' . $this->stage]['profit_tax'] / 100);
          $this->company[$ceo->id]['vat_tax_amount'] = ($this->company[$ceo->id]['uai'] <= 0) ? 0 : - ($this->company[$ceo->id]['total_revenue'] - $this->company[$ceo->id]['total_cost']) * ($this->match->goverment_parameters['stage_' . $this->stage]['vat_tax'] / 100);
        }
        $this->company[$ceo->id]['other_tax_amount'] = ($this->company[$ceo->id]['uai'] <= 0) ? 0 : -$this->company[$ceo->id]['uai'] * ($this->match->match_parameters['easy_business_score_tax'] / 100) * ($this->company[$ceo->id]['market_share'] / 100);
        ...
        $this->global['average_total_cost'] = $this->global['total_cost_sum'] / $this->match->ceos->count();
        ...
        if($this->ceo[$ceo->id]['production'] > 0){
          $this->company[$ceo->id]['event_prob_min'] = ((pow($this->global['risk_slope'], (2 * $this->ceo[$ceo->id]['production'])) / ((1 + 2 * $this->ceo[$ceo->id]['production']) - 1)) * $this->match->match_parameters['risk_limit_min']) / 132;
      ...
      $this->global['average_payed_taxes'] = -1 * ($this->global['taxes_sum'] / $this->match->ceos->count());
      ...
      foreach ($this->match->ceos as $ceo) {
        ...
        if(!$ceo->pivot->bankrupt && !$ceo->pivot->dismissed && $this->stage > 0){
          if($this->company[$ceo->id]['total_funds_ratio'] < $this->match->match_parameters['out_zone']){
      ...
      if($this->stage > 0){
        foreach ($this->match->ceos as $ceo) {
          $tips = new Tips($this->match, $this->global, $this->company[$ceo->id], $ceo);
  ...
  private function init_t0()
  {
    ...
    $match_parameters = $this->match->match_parameters;
    ...
    if ($this->match->match_parameters['type'] != 'country') {
      $match_parameters['salary'] = rand($vars_income_level[$this->match->match_parameters['country_income_level']]['salary_min'], $vars_income_level[$this->match->match_parameters['country_income_level']]['salary_max']);
      $match_parameters['loan_ratio'] = rand($vars_income_level[$this->match->match_parameters['country_income_level']]['loan_ratio_min'] * 1000, $vars_income_level[$this->match->match_parameters['country_income_level']]['loan_ratio_max'] * 1000) / 1000;
    }
    
    $match_parameters['pms'] = rand($vars_income_level[$this->match->match_parameters['country_income_level']]['pms_min'] * 1000, $vars_income_level[$this->match->match_parameters['country_income_level']]['pms_max'] * 1000) / 1000;

    $vars_easy_business_score = json_decode(file_get_contents(__DIR__ . '/data/easy_business_score.json'), true);
    $match_parameters['easy_business_score_tax'] = rand($vars_easy_business_score[$this->match->match_parameters['easy_business_score']]['min'] * 100, $vars_easy_business_score[$this->match->match_parameters['easy_business_score']]['max'] * 100) / 100;
    
    $vars_proficiency_rate = json_decode(file_get_contents(__DIR__ . '/data/proficiency_rates.json'), true);
    $match_parameters['out_zone'] = $vars_proficiency_rate[$this->match->match_parameters['proficiency_rate']]['out_zone'];
    $match_parameters['opening_cash_ratio'] = rand($vars_proficiency_rate[$this->match->match_parameters['proficiency_rate']]['opening_cash_min'], $vars_proficiency_rate[$this->match->match_parameters['proficiency_rate']]['opening_cash_max'] ) / 100;
    $match_parameters['price_t0_leverage'] = rand($vars_proficiency_rate[$this->match->match_parameters['proficiency_rate']]['price_t0_min'], $vars_proficiency_rate[$this->match->match_parameters['proficiency_rate']]['price_t0_max'] ) / 100;
    ...
    if($this->stage == 0){
      ...
      foreach($this->match->ceos as $ceo){
        if($ceo->pivot->bankrupt || $ceo->pivot->dismissed){
          ...
          $params[$ceo->id]['price'] = getMaxPrice($this->match);
  ...
  {
    $industry = $this->match->match_parameters['industry'];
    $vars = json_decode(file_get_contents(__DIR__ . '/industries/' . $industry . '.json'), true);

    $industry_status = $this->match->match_parameters['industry_status'];
